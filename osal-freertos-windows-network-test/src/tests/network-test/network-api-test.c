...
void TestDatagramNetworkApi_Setup(void)
{
    ...
    UtAssert_True(p1_socket_id != 0, "p1_socket_id (%d) != 0", p1_socket_id);

    p1SocketCreator = OS_TaskGetId();
    ...
    /* Set peer1 address */
    actual = OS_SocketAddrFromString(&p1_addr, "192.168.0.4");
    ...
    UtAssert_True(actual == expected, "OS_SocketOpen() (%ld) == OS_SUCCESS", (long)actual);
    UtAssert_True(p2_socket_id != 0, "p2_socket_id (%d) != 0", p2_socket_id);
    ...
    /* Set peer2 address */
    actual = OS_SocketAddrFromString(&p2_addr, "192.168.0.4");
    UtAssert_True(actual == expected, "OS_SocketAddrFromString() (%ld) == OS_SUCCESS", (long)actual);
    ...
    expected = OS_INVALID_POINTER;
    actual   = OS_SocketAddrFromString(NULL, "192.168.0.4");
...
void TestDatagramNetworkApi(void)
{
    ...
    UtAssert_True(actual == expected, "OS_SocketSendTo() Passed. sizeof(Buf1) (%ld) == (%ld)", (long)actual, (long)expected);

    /* Recieve data from peer1 to peer2 */
    expected = sizeof(Buf2);
    actual   = OS_SocketRecvFrom(p2_socket_id, &Buf2, sizeof(Buf2), &l_addr, 100);
    UtAssert_True(actual == expected, "OS_SocketRecvFrom() Passed. sizeof(Buf2) (%ld) == (%ld)", (long)actual, (long)expected);
    ...
    UtAssert_True(actual == expected, "OS_SocketSendTo() Passed. sizeof(Buf3) (%ld) == (%ld)", (long)actual, (long)expected);

    /* Recieve data from peer2 to peer1 */
    expected = sizeof(Buf4);
    actual   = OS_SocketRecvFrom(p1_socket_id, &Buf4, sizeof(Buf4), &l_addr, 100);
    UtAssert_True(actual == expected, "OS_SocketRecvFrom() Passed. sizeof(Buf4) (%ld) == (%ld)", (long)actual, (long)expected);
    ...
    UtAssert_True(PortNum == 9998, "PortNum (%ld) == 9998", (long)PortNum);
    ...
    UtAssert_True(prop.creator == p1SocketCreator, "prop.creator (%d) == p1SocketCreator (%d)", prop.creator, p1SocketCreator);
    UtAssert_True(strcmp(prop.name, "192.168.0.4:9999") == 0, "prop.name (%s) == 192.168.0.4:9999", prop.name);

    actual = OS_SocketGetIdByName(&objid, "192.168.0.4:9999");
    UtAssert_True(actual == expected, "OS_SocketGetIdByName() (%ld) == OS_SUCCESS", (long)actual);
    UtAssert_True(objid == p1_socket_id, "objid (%d) == p1_socket_id (%d)", objid,p1_socket_id);
    ...
    expected = OS_INVALID_POINTER;
    actual   = OS_SocketAddrToString(NULL, 0, &p2_addr);
    UtAssert_True(actual == expected, "OS_SocketAddrToString() (%ld) == OS_INVALID_POINTER", (long)actual);
...
void TestStreamNetworkApi(void)
{
	...
	status = OS_TaskCreate(&s_task_id, "Server", Server_Fn, task_stack, TASK_STACK_SIZE, 23, 0);
    ...
    UtAssert_True(actual == expected, "OS_TimedRead() (%ld) == OS_ERR_INVALID_ID", (long)actual);

    expected = OS_INVALID_POINTER;
    actual   = OS_TimedRead(c_socket_id, NULL, sizeof(Buf_rcv_c), 10);
    UtAssert_True(actual == expected, "OS_TimedRead() (%ld) == OS_INVALID_POINTER", (long)actual);

    expected = OS_ERROR_TIMEOUT;
    actual   = OS_TimedRead(c_socket_id, Buf_rcv_c, sizeof(Buf_rcv_c), 0);
    UtAssert_True(actual == expected, "OS_TimedRead() (%ld) == OS_ERROR_TIMEOUT", (long)actual);

    /* OS_TimedWrite */
    expected = OS_ERR_INVALID_ID;
    temp_id  = 0xFFFFFFFF;
    actual   = OS_TimedWrite(temp_id, Buf_rcv_c, sizeof(Buf_rcv_c), 10);
    UtAssert_True(actual == expected, "OS_TimedWrite() (%ld) == OS_ERR_INVALID_ID", (long)actual);

    expected = OS_INVALID_POINTER;
    actual   = OS_TimedWrite(c_socket_id, NULL, sizeof(Buf_rcv_c), 10);
    UtAssert_True(actual == expected, "OS_TimedWrite() (%ld) == OS_INVALID_POINTER", (long)actual);
    ...
    actual = OS_TimedRead(c_socket_id, Buf_rcv_trans, sizeof(Buf_rcv_trans), 1000);
    UtAssert_True(actual == expected, "OS_TimedRead() (%ld) == %ld", (long)actual, (long)expected);
    UtAssert_True(strcmp(Buf_rcv_trans, Buf_expec_trans) == 0, "Buf_rcv_trans (%s) == Buf_expected (%s)", Buf_rcv_trans,
                  Buf_expec_trans);

    /* Recieve all 256 chars from server one at a time */
    expected = sizeof(Buf_each_char_rcv);
    actual   = OS_TimedRead(c_socket_id, Buf_each_char_rcv, sizeof(Buf_each_char_rcv), 100);
    UtAssert_True(actual == expected, "OS_TimedRead() (%ld) == %ld", (long)actual, (long)expected);
    ...
    /* Once connection socket is closed, verify that no data is received */
	expected = 0;
